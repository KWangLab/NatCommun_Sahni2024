...
icb_binary = on[,-1:-2] %>% set_rownames(on$sample) %>% t(.) %>% .[(row.names(.) %in% AR$interaction),]
...
## ------- Supp 2c -------
sup2c=ggplot(AR_result, aes(x=OR, y=log2(1/ct_FDR), color=group)) + 
  geom_point()  + 
  theme_pubr()+
  ...
  ggtitle('RDI post-treatment') + xlab('odds ratio')
plot(sup2c)
