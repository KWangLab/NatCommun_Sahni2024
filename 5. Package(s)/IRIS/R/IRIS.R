...
## ------- Major Functions -------
#' Leave One Out Cohort Validation
#'
#' function used to perform Leave One Out Cohort (LOOC) validation for IRIS
#' @param test_name string regarding testing/validation cohort name formatted as [cohort]_[pre or on]  (ex. Gide_on, Gide_pre, PUCH_on)
#' @param s1_cohort named list of (data.frame) pre/on-treatment cohort(s) to be used for step1 feature selection; names formatted as [cohort]_[pre/on] (requires at least one pre and one on-rx cohort);  data.frame should have columns sample name (no format requirement), response (values binary 0 or 1), and as many CCI columns (values binary 0 or 1)
#' @param s2_cohort s2_cohort: named list of (data.frame) pre-treatment cohort(s) to be used for step2 feature selection; names formatted as [cohort]_[pre]; data.frame should have columns sample name (no format requirement), response (values binary 0 or 1), and as many CCI columns (values binary 0 or 1)
#' @param direction string ('RDI' or 'RUI') corresponding to searching for RDIs or RUIs
#'
#' @return named list of string of RDIs or RUIs interactions; name correspond to cohort used in step 2 feature selection
#' @export
...
#' Immunotherapy Resistant cell-cell Interaction Scanner
#'
#' function to infer upregulated or downregulated cell-cell interactions that mediate resistance
#' @param s1_cohort named list of (data.frame) pre/on-treatment cohort(s) to be used for step1 feature selection; names formatted as [cohort]_[pre/on] (requires at least one pre and one on-rx cohort); data.frame should have columns sample name (no format requirement), response (values binary 0 or 1), and as many CCI columns (values binary 0 or 1)
#' @param s2_cohort s2_cohort: data.frame of pre-treatment cohort to be used for step2 feature selection; data.frame should have columns sample name (no format requirement), response (values binary 0 or 1), and as many CCI columns (values binary 0 or 1)
#' @param s2_name string of cohort name used for step 2 feature selection. names formatted as [cohort]_[pre]. Input random [cohort] name if you want to reutilize step 1 pre-treatment cohort in step 2 (i.e. RANDOM_pre)
#' @param direction string ('RDI' or 'RUI') corresponding to searching for RDIs or RUIs
#'
#' @return vector of selected interactions (Lcell_Rcell_Ligand_Receptor) from IRIS
#' @export
...
format_s1_input <- function(s2_name, s1_input){
  ...
  meta = colnames(ap) %>% data.frame(sample=.) %>% dplyr::mutate(group = ifelse(sample %in% colnames(s1_on_nr),1,0)) #output 2
...
step1 <- function(s1_input, direction){
  ...
  fisher_output = parallel::mclapply(1:nrow(s1_ap), function(x) step1_fisher(s1_ap[x,], s1_meta)) #fisher test on each interaction individually
  ...
  fisher_output = data.frame(Interaction=row.names(s1_ap), p=fisher_output$p, OR=fisher_output$OR) %>%
    mutate(FDR = p.adjust(p,'fdr')) %>% dplyr::mutate(ct_FDR=NA)
  ...
  ctLR_df = data.frame(fisher_output$Interaction, str_split_fixed(fisher_output$Interaction, '\\_', 4)) %>%
    magrittr::set_colnames(.,c("Interaction", "Lcell", "Rcell", "L", "R")) %>% dplyr::mutate(Lcell_Rcell=paste(Lcell,Rcell,sep='_'))
  ...
  if (direction == 'RDI'){
    print('RDI direction')
    s1_rank = fisher_output %>% subset(ct_FDR < 0.2 & OR < 1) %>% dplyr::arrange(ct_FDR)
...
step1_fisher <- function(activity_profile, meta){ #function for fisher-exact test regarding enrichment of activation
  ...
  res = data.frame(p=stats::fisher.test(mat)$p.value, OR = stats::fisher.test(mat)$estimate) #fisher-exact test
...
step2 <- function(s1_output, s2_ap, s2_meta, trial=500, direction){
  ...
  fs_matrix = parallel::mclapply(1:trial, function(i){ #develop 2D feature score (0;1;-1) matrix iterating forward feature selection algorithm over n trials

    print(i) # trial i

    #1. partition step 2 training cohort into 2 training and 1 testing fold
    fold = rBayesianOptimization::KFold(c(1:nrow(s2_meta)), nfolds=3, seed=i)[[1]]
...
## ------- Major Functions -------
#' function used to develop ensemble score following Sahni et al. 2024 paper
#'
#' @param test data.frame regarding testing/validation cohort (to apply features): data.frame should have columns sample name (no format requirement), response (values binary 0 or 1), and as many CCI columns (values binary 0 or 1)
#' @param test_name string regarding testing/validation cohort name formatted as [cohort]_[pre or on]  (ex. Gide_on, Gide_pre, PUCH_on)
#' @param features LOOCohort_validation or IRIS output
#' @param sample_name boolean whether sample name should be included in final output or not
#'
#' @return data.frame of patient ensemble score using IRIS features
#' @export
calculate_score <- function(test, test_name, features, sample_name=F){
  input_name = strsplit(test_name, '_') %>% unlist(.) %>% .[[1]]
  ...
  ensembl_df=lapply(1:length(train), function(r){ #calculate score for each ensembl model
    model_score = test %>% dplyr::select(-sample, -response) %>% .[colnames(.) %in% train[[r]]] %>% apply(1, sum)
  ...
  if (sample_name == T){
    score_df = ensembl_df %>% apply(1,function(x){sum(x,na.rm=T)}) %>% data.frame(cohort = test_name, sample = test[,c("sample")], response=test[,c("response")], score=c(./length(train)))
    return(score_df)
  }

  score_df = ensembl_df %>% apply(1,function(x){sum(x,na.rm=T)}) %>% data.frame(response=test[,c("response")], score=c(./length(train)))
  return(score_df)
}
