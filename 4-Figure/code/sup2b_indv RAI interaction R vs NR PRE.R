...
icb_binary = pre[,-1:-2] %>% set_rownames(pre$sample) %>% t(.) %>% .[(row.names(.) %in% R$interaction),]
...
R_result = data.frame(interaction=row.names(icb_binary)) %>% cbind(., interaction_result) %>% mutate(OR=OR)
...
## ------- Supp 2b -------
sup2b=ggplot(R_result, aes(x=OR, y=log2(1/ct_FDR), color=group)) + 
  geom_point()  + 
  theme_pubr()+
