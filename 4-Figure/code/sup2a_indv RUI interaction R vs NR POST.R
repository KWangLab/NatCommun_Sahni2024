...
R_LIRICS = feature_RUI
R=R_LIRICS %>% unlist(.)  %>% table(.) %>% as.data.frame(.) %>% set_colnames(c('interaction','freq'))

## organize input
on=do.call(dplyr::bind_rows, model_input[[1]][c(1,4,7)]) #only on-treatment

icb_meta = on %>% select(sample=sample, response=response)
icb_binary = on[,-1:-2] %>% set_rownames(on$sample) %>% t(.) %>% .[(row.names(.) %in% R$interaction),]
...
R_result = data.frame(interaction=row.names(icb_binary)) %>% cbind(., interaction_result) %>% mutate(OR)

Cell_LR_df = data.frame(R_result$interaction, str_split_fixed(R_result$interaction, '\\_', 4)) %>%
  set_colnames(.,c("Interaction", "Lcell", "Rcell", "L", "R")) %>% mutate(Lcell_Rcell=paste(Lcell,Rcell,sep='_'))

R_result = merge(R_result, Cell_LR_df, by.x=1, by.y=1) %>% mutate(ct_FDR=NA) %>% mutate(FDR=p.adjust(p,'fdr'))

## cell-pair-specific p-value adjustment.
cell.pair.counts <- R_result$Lcell_Rcell %>% table %>% sort(decreasing = T)       # counts of all cell pairs present
for (cc in cell.pair.counts %>% names){
  idx <- which(R_result$Lcell_Rcell == cc)                                      # indices for the specific cell pair
  R_result[idx, "ct_FDR"] <- R_result[idx, "p"] %>% 
    p.adjust(method = "fdr")
}

R_result = R_result %>% mutate(group=ifelse(ct_FDR<0.2 & OR < 1, 'NR','NS'))

## ------- Supp 2A -------
sup2A=ggplot(R_result, aes(x=OR, y=log2(1/ct_FDR), color=group)) + 
  geom_point()  + 
  theme_pubr()+
